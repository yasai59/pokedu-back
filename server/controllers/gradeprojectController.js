...
export const gradeprojectDelete = async (req, res) => {
  ...
  try {
    result = await dbQuery(
      `DELETE FROM USUARIOS_PROJECTES WHERE id = '${gradeprojectId}';`
    );
  } catch (e) {
    console.log(e);
    return res.status(400).json({
...
export const gradeprojectGet = async (req, res) => {
  ...
  try {
    result = await dbQuery(
      `SELECT * FROM USUARIOS_PROJECTES WHERE id = ${gradeprojectId};`
    );
  } catch (e) {
    return res.status(400).json({
      error: "Invalid query",
...
export const gradeprojectPut = async (req, res) => {
  ...
  try {
    result = await dbQuery(
      `UPDATE USUARIOS_PROJECTES SET alumne = '${gradeprojectAlumne}' WHERE id = '${gradeprojectId}'``UPDATE USUARIOS_PROJECTES SET projecte = '${gradeprojectProjecte}' WHERE id = '${gradeprojectId}'`
    );
  } catch (e) {
    console.log(e);
    return res.status(400).json({
